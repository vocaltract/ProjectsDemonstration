...
static OpcodeEntry opcode_table [512] = {//通过宏，直接把结构的内容写出来
  ...
  /* 0xb8 */	IDEX(mov_I2r, mov), IDEX(mov_I2r, mov), IDEX(mov_I2r, mov), IDEX(mov_I2r, mov),//{decode_moveI2r, exec_move,0}
...
/*2 byte_opcode_table */
  ...
  /* 0x20 */	IDEX(cr2r, mov_cr2r), EMPTY, IDEX(r2cr, mov_r2cr), EMPTY,
...
void isa_exec(vaddr_t *pc) {
  ...
  decinfo.opcode = opcode;
  set_width(opcode_table[opcode].width);//输入为0时，decinfo.isa.is_operand_size_16有就是2,没有就是4;记录在decinfo
  //否则decinfo.src.width = decinfo.dest.width = decinfo.src2.width = width;
  idex(pc, &opcode_table[opcode]);//传指针以提高效率
}
