 ...
 */
/* sign immediate */
static inline make_DopHelper(SI) {//void decode_op_SI (vaddr_t *pc, Operand *op, bool load_val){
// decode_op_SI(pc, id_dest, false);是对于该函数的调用,所以pc是&decinfo.seq_pc;op为&decinfo.dest;load_val=false
  ...
  rtl_sext(&op->imm, &op->imm, op->width);//先把有符号数当作无符号数进行传递
...
static inline make_DopHelper(r) {
  op->type = OP_TYPE_REG;//一个枚举类型
  op->reg = decinfo.opcode & 0x7;//取低三位
  if (load_val) {
    rtl_lr(&op->val, op->reg, op->width);//把编号为op->reg的寄存器的值读入op->val中
  }
...
static inline make_DopHelper(O) {
  ...
  if (load_val) {
    rtl_lm(&op->val, &op->addr, op->width);//把第二个参数作为地址，把width字节个东西读入第一个参数中
  }
...
make_DHelper(mov_I2r) {
  decode_op_r(pc, id_dest, false);//false是否load_val
  decode_op_I(pc, id_src, true);
}
...
make_DHelper(J) {//void decode_J (vaddr_t *pc){   pc是&decinfo.seq_pc
...
make_DHelper(r2cr){
  decode_op_rm(pc, id_src, true, id_dest, false);//这玩意儿跟正常的是反过来的，R/M是源操作数，reg是目的操作数
}

make_DHelper(cr2r){
  decode_op_rm(pc, id_dest, false, id_src, false);//这玩意儿跟正常的是反过来的，R/M是源操作数，reg是目的操作数
}


void operand_write(Operand *op, rtlreg_t* src) {
  if (op->type == OP_TYPE_REG) { rtl_sr(op->reg, src, op->width); }//写入寄存器
  else if (op->type == OP_TYPE_MEM) { rtl_sm(&op->addr, src, op->width); }//写入内存
  else { assert(0); }
}
