...
NEMUState nemu_state = {.state = NEMU_STOP};//include/monitor/monitor.h  I guess NEMU_STOP is equivalent to a static variable
...
vaddr_t exec_once(void);//return the new pc, I guess
...
void cpu_exec(uint64_t n) {
  ...
  if (g_nr_guest_instr < LOG_MAX) {
    ...
    bool whether_change = update_all_w();
    if (whether_change)
    {
